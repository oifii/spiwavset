	...
	*/
	...
	if(!tempwavsetvector.empty())
	{
		...
		else
		{
			...
			if(1)
			{
				printf("warning, %d wavsets matching midi note number %d\n", tempwavsetvector.size(), midinotenumber);
			}
			#endif //_DEBUG
			int random_integer;
	...
	else
	{
		...
		if(1)
		{
			printf("warning, no wavsets matching midi note number %d\n", midinotenumber);
		}
		#endif //_DEBUG
		int idwavset = wavsetvector.size()*midinotenumber/128;
		pWavSet = wavsetvector.at(idwavset);
...
bool Instrument::CreateWavSynth(int flag)
{
	...
	if(flag==INSTRUMENT_SYNTH_SINWAV)
	{
		...
		for(int i=0; i<128; i++)
		{
			...
			{
				cout << "error in Instrument::CreateWavSynth(), not enough memory?\n" << " failed at id=" << i << endl;
				return false;
			}
		}
	}
