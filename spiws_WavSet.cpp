...
{
	assert(playerflag==USING_SOX || playerflag==USING_SPIPLAY);
	...
	if(playerflag==USING_SOX)
	{
		...
		int numRepeat = 1;
		//sprintf_s(charBuffer3,WAVSET_CHARNAME_MAXLENGTH-1,"start %s D:\\oifii-org\\httpdocs\\ns-org\\nsd\\ar\\cp\\audio_spi\\app-bin\\sox\\sox.exe %s \"%s\" -d repeat %d",charBuffer5,charBuffer6,filename.c_str(),numRepeat);
